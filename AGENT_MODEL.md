...
and — for this repository — the dictation app itself, which requires
a mic, hotkey listener, and GPU on the admin machine), and those pull
their own latest version from GitHub before every run.
...
automation on the admin machine, and (for this repository) running
and testing the dictation app itself against a real mic/GPU — are
still routed to Cowork or to Kevin directly via a surgical brief.
Claude Code cannot execute these remotely.
...
2. Cowork (or Kevin) executes the brief on the admin machine.
...
in CLAUDE.md. This repository (windows-dictation) is private —
no GitHub Pages hosting is required, so there is no reason to make
it public.
...
| windows-dictation     | Active         | New — local Windows dictation app, no GitHub Pages, private |
...
|         |            | Lelitte 2026-07-02.                 |
| —       | 2026-07-09 | (windows-dictation local copy)      |
|         |            | Section 1 and Section 7 annotated   |
|         |            | for this repo's local-execution     |
|         |            | nature; Section 8 row added. Not a  |
|         |            | constitutional amendment — this is  |
|         |            | this repo's copy of a cross-repo    |
|         |            | document; propagate to the shared   |
|         |            | table in other repos separately if  |
|         |            | Kevin wants full-estate visibility. |
